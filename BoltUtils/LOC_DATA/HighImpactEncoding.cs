...
namespace BoltUtils.LOC_DATA
{
    ...
    public class HighImpactEncoding : Encoding
    {
        ...
        /// <summary>
        /// Latin extension table.
        /// </summary>
        /// <remarks>https://en.wikipedia.org/wiki/Latin_Extended-A</remarks>
        private readonly Dictionary<string, byte[]> SymbolToBytesTableVariantLatinExtended = new Dictionary<string, byte[]>
        {
            ["Ą"] = [0x93, 0xC0],
            ["Ę"] = [0x93, 0xC1],
            ["Ć"] = [0x93, 0xC2],
        ...
        /// <summary>
        /// Japanese extension table.
        /// </summary>
        private readonly Dictionary<string, byte[]> SymbolToBytesTableVariantJapanese = new Dictionary<string, byte[]>
        {
            ["、"] = [0x93, 0x01],
            ["。"] = [0x93, 0x02],
            ["「"] = [0x93, 0x0C],
        ...
        /// <summary>
        /// Symbol → byte[] table. Size Matters Demo format version only.
        /// </summary>
        private readonly Dictionary<string, byte[]> SymbolToBytesTableSMDemo = new Dictionary<string, byte[]>
        {
            // Buttons and special icons
            ["<special=unused/>"] = [0xB1],
            ["<button=x/>"] = [0xB8],
            ["<button=square/>"] = [0xB9],
            ["<button=triangle/>"] = [0xBA],
            ["<button=circle/>"] = [0xBB],
            ["<button=joystick/>"] = [0xBD],
            ["<button=d-pad_up/>"] = [0xCE],
            ["<button=d-pad_down/>"] = [0xCF],
            ["<button=d-pad_left/>"] = [0xD0],
            ["<button=d-pad_right/>"] = [0xD1],
            ["<symbol=registered/>"] = [0xD5],
            ["<button=d-pad_all/>"] = [0xD7],
            ["<button=volume_up/>"] = [0xCA],
            ["<button=volume_down/>"] = [0xCB],
            ["<button=select/>"] = [0xC0],
            ["<button=start/>"] = [0xC1],
            ["<button=l/>"] = [0xBE],
            ["<button=r/>"] = [0xBF],
        ...
        /// </summary>
        public HighImpactEncoding(EncodingVariant variant = EncodingVariant.LatinExtended, Version version = Version.SM_SAC_RETAIL)
        {
            if (version == Version.SM_SAC_RETAIL || version == Version.SM_PROTO)
            {
                // Add the corresponding extended character page
                switch (variant)
                {
                    case EncodingVariant.LatinExtended:
                        {
                            SymbolToBytesTable = SymbolToBytesTable.Concat(SymbolToBytesTableVariantLatinExtended).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
                            break;
                        }

                    case EncodingVariant.Japanese:
                        {
                            SymbolToBytesTable = SymbolToBytesTable.Concat(SymbolToBytesTableVariantJapanese).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
                            break;
                        }
                }
                ...
                {
                    SymbolToBytesTable.Add($"<icon={i}/>", [0x95, i]);
                }
            ...
            // The SM demo has a different set of IDs for buttons and lacks features.
            else if (version == Version.SM_DEMO)
            {
                SymbolToBytesTable = SymbolToBytesTableSMDemo;
    ...
    /// <summary>
    /// Variant of the <see cref="HighImpactEncoding"/>.
    /// </summary>
